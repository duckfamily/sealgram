...
Если прокси умирает, фото и видео перестают грузиться или ползут —
тихо переключается на другой. Ничего настраивать не нужно.
...
Удалённые сообщения остаются в чате с пометкой, а у изменённых
видна история правок.
...
Собран на Telegram Desktop 7.2.9: новые функции Telegram работают,
а не показываются как «не поддерживается».
...
| **Ротация прокси** | Смена при зависании основного соединения и отдельно — при зависании или медленной загрузке медиа (медиана скорости реальных загрузок ниже 48 КБ/с): прокси может пропускать сообщения и резать медиа. Прокси, плохо отдававшие медиа, пропускаются 30 минут, в том числе после перезапуска. |
| **Режим шпиона** | Удалённые на сервере сообщения остаются в чате с пометкой до перезапуска; список удалённых и история правок читаются из зашифрованного локального архива. |
